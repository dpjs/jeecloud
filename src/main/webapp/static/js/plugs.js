define(['jquery'], function () {
    ...
    function form() {
        ...
        this.iframe = function (url, title) {
            ...
            return layer.open({title: title || '窗口', type: 2, area: area, fix: true, maxmin: true, content: url,
            	success: function (layero, index) {
                    var that = this;
                    $.form.layerfooter(layero, index, that);
                }});
        };
        this.layerfooter= function (layero, index, that) {
            var frame = $.form.getChildFrame('html', index);
        ...
        this.getChildFrame = function(selector, index){
        	...
        	return $('#layui-layer' + index).find('iframe').contents().find(selector);  
        };
        // 加载HTML到弹出层
